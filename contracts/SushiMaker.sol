...
// will receive LP tokens from people trading on SushiSwap
contract SushiMaker {
  ...
  function convert(address token0, address token1) public {
    ...
    // Burn the LP tokens for the provided token pair
    // The result will be receiving proportional amounts of both token0 and token1.
  ...
  function _toWETH(address token) internal returns (uint256) {
    ...
    if (token == weth) {
      ...
      // sending it to the SUSHI/WETH pair for subsequent conversion
